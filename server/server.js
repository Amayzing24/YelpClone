...
app.get("/api/v1/restaurants/search/:searchText", async (req, res) => {
    try {
        const searchQuery = req.params.searchText.replaceAll(' ', '&').replaceAll(/'/g, '')
        const response = await db.query("SELECT * FROM restaurants WHERE to_tsvector(name) @@ to_tsquery('" + searchQuery + "')")
